	         ...
	         <table class="table table-hover table-striped table-bordered"  id="messageTable">
				...
				<tbody>
					...
					<tr class="{{$message->status==0 ? 'unread_message' : ''}} ">
						...
						<td><a class="" href="{{route('message.show',['id'=>$message->id])}}">{{$count++}}</a></td>
						<td><a class="" href="{{route('message.show',['id'=>$message->id])}}">{{$message->name}}</a></td>
						<td><a class="" href="{{route('message.show',['id'=>$message->id])}}">{{$message->subject}}</a></td>
						<td><a class="" href="{{route('message.show',['id'=>$message->id])}}">{{mb_substr($message->message,0,40).'...'}}</a></td>
						<td><a  class="m-0"  href="{{route('message.show',['id'=>$message->id])}}">{{ isset($message->created_at) ? $message->created_at->format('d M, Y') : '' }}
					<p class="pull-right text-muted">{{$message->created_at->format('H:i')}}</p></a></td>
