						...
						<form action="{{route('users.update',$user->id)}}" method="post">
									 ...
									 <select name="user_catg_id" class="form-control" id="user_catg_id"  >
		                                <option value="0">Select User Type</option>
			                                @foreach($roles as $role)
			                                    <option value="{{ $role->id}}" {{old('user_catg_id')== $role->id ? 'selected' : ''}} {{$user->user_catg_id == $role->id ? 'selected' : ''}}>{{ $role->display_name}}</option>
			                                @endforeach
		                               </select>
							...
							<div class="row form-group">
								<div class="col-md-6">
									...
									<input type="text" class="form-control timepicker" name="mobile" value="{{old('mobile') ?? $user->mobile}}"> 
									@error('mobile')
	                                    <span class="text-danger">
	                                        <strong>{{ $message }}</strong>
	                                    </span>
