...
@section('content')
		...
		<div class="row">
			<div class="col-md-12">
				<div class="box box-primary">
					...
					<div class="box-body">
						<form action="{{route('court_subcategory.update',['id' => $court->court_code])}} " method="post">
							@csrf
							@method('PATCH')
							<div class="row form-group">
								<div class="col-md-6">
									...
									<select name="court_group_code" class="form-control" id="court_group">
										<option value="0">Select Group Name <span class="text-danger">*</span></option>
										@foreach($courtgrups as $courtgrup)
											<option value="{{$courtgrup->court_group_code}}" {{old('court_group_code') == $courtgrup->court_group_code ? 'selected' : '' }} {{$court->court_group_code == $courtgrup->court_group_code ? 'selected' : ''}}>{{$courtgrup->court_group_name}}</option>
										@endforeach
									</select>
							...
							<div class="row form-group">
								<div class="col-md-6">
									...
									<input type="text" name="court_name" value="{{old('court_name') ?? $court->court_name}}" class="form-control" >
									@error('court_name')
										<span class="text-danger">
											<strong>{{$message}}</strong>
										</span>
								...
								<div class="col-md-6">
									...
									<input type="text" name="court_shrt_name" value="{{old('court_shrt_name') ?? $court->court_shrt_name}}" class="form-control">
	...
	$(document).ready(function(){
		...
		var court_group_code = $('#court_group').val();;
		var court_type = "{{$court->court_type}}";
		    ...
		    $.ajax({ 
		      ...
		      {   	
		          ...
		          if(res.length !=0){
		              ...
		              $("#court_type").append('<option value="0">Select Court Type</option>');
		              $.each(res,function(key,value){
		                  $("#court_type").append('<option value="'+value.court_type+'" '+(value.court_type == court_type ? 'selected="selected"' : '')+' '+(value.court_type == oldcourt_type ? 'selected="selected"' : '')+'>'+value.court_type_desc+'</option>');
		              });
		          }else{
		              $("#court_type").empty();		              
		          }
