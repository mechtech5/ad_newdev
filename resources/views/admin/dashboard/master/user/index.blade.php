			...
			<div class="col-md-12">
				<div class="box box-primary">
					<div class="box-header">
						<h3 class="">Users <a href="{{route('user.create')}}" class="btn btn-sm btn-primary pull-right">Add User</a></h3>
					</div>
							...
							<tbody>
								...
								@foreach($users as $user)
									<tr>
										...
										<td>{{$user->country != null ? $user->country['country_name']  : '-'}}</td>
										...
										<td>{{date('d-m-y', strtotime($user->created_at))}}</td>
										 <td>
										 	<form action="{{route('user.destroy', ['id' =>  $user->id ])}}" method="POST" id="delform_{{ $user->id }}">
											@method('DELETE')

										 	<a href="{{route('user.edit',$user->id)}}"><i class="fa fa-edit text-green btn btn-sm"></i></a>
